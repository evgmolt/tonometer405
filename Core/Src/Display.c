...
void PrintBattCharge(void)
{
    ...
    HAL_ADC_Start(&hadc1);
    HAL_ADC_PollForConversion(&hadc1, 100); // îæèäàåì îêîí÷àíèÿ ïðåîáðàçîâàíèÿ
    batt_level = HAL_ADC_GetValue(&hadc1); // ÷èòàåì ïîëó÷åííîå çíà÷åíèå 
    HAL_ADC_Stop(&hadc1);
...
void PrintError(uint8_t K){
    ...
    if (K==ERROR_MEAS)
    {
        sprintf(_buff,"ÎØÈÁÊÀ %d %d %d", p_sys, p_dia, pulse);        
//        sprintf(_buff,"ÎØÈÁÊÀ ÈÇÌÅÐÅÍÈß");        
        ILI9341_WriteString(left_margin, top_margin, _buff, Font_Arial, ILI9341_RED, ILI9341_WHITE);            
    }        
}
